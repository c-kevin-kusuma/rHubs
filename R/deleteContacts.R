...
#' @param contactId Values that represent the internal IDs of the contacts on your 'HubSpot' instance.
#' @examples output <- deleteCompanies(
#'   accessToken = accessToken,
#'   contactId = contactId)
#' @export

deleteContacts <- function(accessToken, contactId) {
  ...
  for (i in 1:length(vid)) {
    hubSpotOutput <- httr::DELETE(url = paste0('https://api.hubapi.com/crm/v3/objects/contacts/', contactId[i]), httr::add_headers(.headers = c("authorization"=paste("Bearer", accessToken), "Content-Type"="application/json")))

    outputContent <- hubSpotOutput %>% httr::content()
    statusCode <- hubSpotOutput %>% httr::status_code()

    if(statusCode == 204) {deleteList[[i]] <- dplyr::tibble(rowid = i, vid = vid[i], statusCode = statusCode, message = 'The contact has been deleted')} else {deleteList[[i]] <- dplyr::tibble(rowid = i, vid = vid[i], statusCode = statusCode, message = outputContent$message)}
  }
