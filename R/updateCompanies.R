...
updateCompanies <- function(accessToken, data){
  options(scipen=999)
  ...
  if ('hs_object_id' %in% colnames(data)) {} else {stop('A required \"hs_object_id\" field that represents the internal ID of the company is missing from the \"data\".', call. = FALSE)}
  ...
  for (i in 1:nrow(data)) {
    ...
    hubSpotOutput <- httr::PATCH(url = paste0('https://api.hubapi.com/crm/v3/objects/companies/', hs_object_id), body = json, httr::add_headers(.headers = c("authorization"=paste("Bearer", accessToken), "Content-Type"="application/json")))
